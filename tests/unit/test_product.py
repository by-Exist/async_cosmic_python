...
def test_prefers_warehouse_batches_to_shipments():
    in_stock_batch = Batch(
        reference="in-stock-batch", sku="RETRO-CLOCK", _purchased_quantity=100, eta=None
    )
...
def test_outputs_allocated_event():
    batch = Batch(
        reference="batchref", sku="RETRO-LAMPSHADE", _purchased_quantity=100, eta=None
    )
...
def test_records_out_of_stock_event_if_cannot_allocate():
    batch = Batch(
        reference="batch1", sku="SMALL-FORK", _purchased_quantity=10, eta=today
    )
...
def test_increments_version_number():
    ...
    product = Product(
        sku="SCANDI-PEN",
        batches=[
            Batch(reference="b1", sku="SCANDI-PEN", _purchased_quantity=100, eta=None)
        ],
    )
